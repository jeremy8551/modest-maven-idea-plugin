    ...
    public void merge(List<SearchEverywhereFoundElementInfo> infos, boolean keepOtherContributorItem) {
        if (keepOtherContributorItem) {
            ...
            for (int i = 0; i < model.getSize(); i++) {
                ...
                if (!(element instanceof SearchNavigation) && !model.isMoreElement(i)) {
                    all.add(info);
                }
            }
            ...
            if (!infos.isEmpty()) {
                boolean add = true;
                int priority = CollectionUtils.first(infos).getPriority();
                for (int i = 0; i < all.size(); i++) {
                    SearchEverywhereFoundElementInfo elementInfo = all.get(i);
                    if (elementInfo != null && elementInfo.getPriority() <= priority) {
                        all.addAll(i, infos);
                        add = false;
                        break;
                    }
