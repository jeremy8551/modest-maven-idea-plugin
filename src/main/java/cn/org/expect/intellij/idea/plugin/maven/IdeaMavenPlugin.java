    ...
    public VirtualFile getSettingsXml(AnActionEvent event) {
        ...
        if (manager != null) {
            ...
            if (generalSettings != null) {
                ...
                String userSettingsFile = generalSettings.getUserSettingsFile();

                try {
                    userSettings = MavenWslUtil.getUserSettings(event.getProject(), userSettingsFile, generalSettings.getMavenConfig());  // settings.xml
                } catch (NoClassDefFoundError e) {
                    if (FileUtils.isFile(userSettingsFile)) {
                        userSettings = new File(userSettingsFile);
                    }

                    if (userSettings == null) {
                        File repository = this.getRepository(manager);
                        File settingsXml = new File(repository.getParentFile(), "settings.xml");
                        if (FileUtils.isFile(settingsXml)) {
                            userSettings = settingsXml;
                        }
                    }
                }
